...
class Testbench(object):
    ...
    def return_tcl_script(self):
        ...
        if self.settings.coverage:
            if self.bus.bus_type == 'axi':
                s += ('coverage exclude -du {0}_{1}_pif -togglenode araddr\n'
    ...
    def check_default_value(self, reg):
        ...
        if reg.sig_type in ['sl', 'slv', 'default']:
            ...
            for field in reg.fields:
                ...
                s += self.check_value(signal_field, self.sig_value(field.reset, field.sig_type, field.length), msg_field)
    ...
    def set_check_zero_value(self, reg):
        ...
        if reg.mode == 'rw':
            ...
            if reg.sig_type in ['sl', 'slv', 'default']:
                ...
                for field in reg.fields:
                    signal_field = '{}.{}'.format(signal, field.name)
                    s += self.check_value(signal_field, self.sig_value('0', field.sig_type, field.length), msg)
            else:
            ...
            if reg.sig_type in ['sl', 'slv', 'default']:
                ...
                for field in reg.fields:
                    signal_field = '{}.{}'.format(signal, field.name)
                    s += '{} <= {};\n'.format(signal_field, self.to_zero(field.sig_type))
                for field in reg.fields:
                    signal_field = '{}.{}'.format(signal, field.name)
        ...
        if reg.mode == 'rw':
            if reg.sig_type == 'sl':
                ...
                for field in reg.fields:
                    ...
                    if field.sig_type == 'sl':
                        s += self.check_bit_rw_sl(c_addr, signal_field, msg, field.pos_low)
                    elif field.sig_type in ['slv', 'default']:
                        s += self.check_bit_rw_slv(c_addr, signal_field, field, msg, field.pos_low)
                    s += '\n'
            else:
                raise Exception("Invalid register type")
        elif reg.mode == 'ro':
            if reg.sig_type == 'sl':
                ...
                for field in reg.fields:
                    ...
                    s += '\n'
                    if field.sig_type == 'sl':
                        s += self.check_bit_ro_sl(c_addr, signal_field, msg, field.pos_low)
                    elif field.sig_type in ['slv', 'default']:
                        s += self.check_bit_ro_slv(c_addr, signal_field, field, msg, field.pos_low)
                    s += '\n'
            else:
                raise Exception("Invalid register type")
