...
class Test_Suite(unittest.TestCase):
    ...
    def test_pif(self):
        for holder in self.buzz:
            with self.subTest(holder=holder):
                ...
                with open('example/example_{0}/hdl/example_{0}_pif_pkg.vhd'.format(holder.bus.bus_type)) as f:
                    string = f.read()
                self.assertEqual(holder.mod.return_module_pkg_VHDL(), string, "pif pkg must match manual file")

                with open('example/example_{0}/hdl/example_{0}_{1}_pif.vhd'.format(holder.bus.bus_type, holder.bus.short_name)) as f:
                    string = f.read()
                self.assertEqual(holder.bus.return_bus_pif_VHDL(holder.mod), string, "pif must match manual file")

    def test_module(self):
        for holder in self.buzz:
            with self.subTest(holder=holder):
                self.logger.info("Testing Module generation...")
                with open('example/example_{0}/hdl/example_{0}.vhd'.format(holder.bus.bus_type)) as f:
                    string = f.read()
                self.assertEqual(holder.mod.return_module_VHDL(), string, "module must match manual file")
    ...
    def test_buspkg(self):
        ...
        with open('example/example_axi/hdl/axi_pkg.vhd') as f:
            string = f.read()
        self.assertEqual(holder.bus.return_bus_pkg_VHDL(), string, "bus pkg must match manual file")
    ...
    def test_bench_scripts(self):
        for holder in self.buzz:
            with self.subTest(holder=holder):
                ...
                with open('example/example_{}/scripts/component_list.txt'.format(holder.bus.bus_type)) as f:
                    string = f.read()
                self.assertEqual(holder.tb.return_uvvm_component_list(), string, ".txt file must match manual file")

    def test_bench_hdl(self):
        for holder in self.buzz:
            with self.subTest(holder=holder):
                self.logger.info("Testing TB Sequencer generation...")
                with open('example/example_{0}/tb/example_{0}_{1}_pif_tb.vhd'.format(holder.bus.bus_type, holder.bus.short_name)) as f:
                    string = f.read()
                self.assertEqual(holder.tb.return_vhdl_tb(), string, ".vhd TB must match manual file")
    ...
    def test_hdr(self):
        for holder in self.buzz:
            with self.subTest(holder=holder):
                ...
                with open('example/example_{0}/header/example_{0}.h'.format(holder.bus.bus_type)) as f:
                    string = f.read()
                self.assertEqual(holder.hdr.return_c_header(), string, "c header must match manual file")

                with open('example/example_{0}/header/example_{0}.hpp'.format(holder.bus.bus_type)) as f:
                    string = f.read()
                self.assertEqual(holder.hdr.return_cpp_header(), string, "c++ header must match manual file")
